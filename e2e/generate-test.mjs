...
const packageNames = await fs.readdir(new URL('../packages/@Test', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@Test/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@Test/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('Test', 2)[1]
