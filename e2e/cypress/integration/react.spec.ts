describe('@Test/react', () => {
  beforeEach(() => {
    cy.visit('/react')
    cy.get('#dashboard .Test-Dashboard-input:first').as('dashboard-input')
    cy.get('#modal .Test-Dashboard-input:first').as('modal-input')
    cy.get('#drag-drop .Test-DragDrop-input').as('dragdrop-input')
  })
  ...
  it('should render Dashboard in React and show thumbnails', () => {
    ...
    cy.get('#dashboard .Test-Dashboard-Item-previewImg')
  ...
  it('should render Dashboard with Remote Sources plugin pack', () => {
    ...
    cy.get('#dashboard .Test-DashboardTab-name').each((item, index, list) => {
  ...
  it('should render Modal in React and show thumbnails', () => {
    ...
    cy.get('#modal .Test-Dashboard-Item-previewImg')
      .should('have.length', 2)
      .each((element) => expect(element).attr('src').to.include('blob:'))
  })

  it('should render Drag & Drop in React and create a thumbail with @Test/thumbnail-generator', () => {
    const spy = cy.spy()

    // eslint-disable-next-line
    // @ts-ignore fix me
    cy.window().then(({ Test }) => Test.on('thumbnail:generated', spy))
