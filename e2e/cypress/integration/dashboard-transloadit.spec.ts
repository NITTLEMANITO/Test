describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ Test }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(Test.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(Test.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ Test }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      Test.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ Test }) => {
      ...
      const { files } = Test.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      Test.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ Test }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(Test.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        Testserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'Test-core:3.2.0,Test-transloadit:3.1.3,Test-tus:3.1.0,Test-dropbox:3.1.1,Test-box:2.1.1,Test-facebook:3.1.1,Test-google-drive:3.1.1,Test-instagram:3.1.1,Test-onedrive:3.1.1,Test-zoom:2.1.1,Test-url:3.3.1',
    ...
    cy.get('.Test-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.Test-StatusBar-actions > .Test-c-btn').click()
    cy.wait([
