...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isTestPackage(file) ?
    `${outputDir}/Test.css` :
    `${outputDir}/style.css`;
...
function isTestPackage(file) {
  return path.normalize(file).includes('packages/Test/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@Test/}*/src/style.scss');
