...
This example uses @Test/companion with a custom AWS S3 configuration.
...
`@Test/companion` in a `.env` file. You may find useful to first copy the
`.env.example` file:
...
corepack yarn workspace @Test-example/aws-companion start
```
